...
export default function Canvas() {
  ...
  useEffect(() => {
    if (canvasRef.current && wrapperRef.current) {
      ...
      activeTemplate.config.forEach(async (config) => {
        const PROPERTIES = config.rectFabric(ratio.height, ratio.width)
        const cell = new fabric.Rect(PROPERTIES).set(OBJECT_LOCKED)
        const deafult = fabric.Image.fromURL("src/components/Icon/image.png")
        const deafultimgId = `img_${new Date().getTime()}`
        ...
        ;(await deafult).set({
          id: deafultimgId,
          left: cell.left + (cell.width - 140) / 2,
          top: cell.top + (cell.height - 150) / 2,
          selectable: false,
          hasControls: false,
